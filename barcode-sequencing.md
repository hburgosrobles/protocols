...
**Important:** Throughout this protocol the assumption is that you will be working with a high number of samples so that most procedures will be happening in 96-well plates/arrangements. To make this process easier and increase the chances of success, whenever necessary, fill out a 96-well worksheet with your sample layout that you can use as a guide. - [96-well worksheet](images/barcode-sequencing/96-well-worksheet.xlsx)
...
2. Quantify DNA with Quant-iT 96-well kit following the [manufacturer's protocol](images/barcode-sequencing/User-Guide_Quant-iT-dsDNA-High-Sensitivity-Assay-Kit.pdf) for the Quant-iT dsDNA High-Sensitivity Assay Kit with a Fluorescence Microplate Reader with the following additions/notes:

  a) Make the 'working solution' (step 1.1) in a 50 mL conical tube and transfer 200 µl (use M4 repeater pipette) to each well of a Nunc 96-well flat bottom plate with black walls. - make enough working solution to account for your unknown samples, the standards, and a few extra samples to ensure you have enough volume available after pipetting/in case of some errors.

  b) In duplicate wells, add 10 µl of the λ standards as described.

  c) Add **5 µl** (protocol allows for 1 - 20 µl) of your unknown samples to the wells - it is **strongly** recommended that you prepare a 96-well layout sheet with you sample arrangements filled out to use as a guide when loading the samples/standards - [example 96-well layout](images/barcode-sequencing/2021-05-04_eHB94-I_Quant-iT-gDNA.xlsx). Include blank wells that have only the working solution.

  d) Measure fluorescence on a plate reader (in the Mandel lab we use the Synergy neo2 multi-mode reader from Biotek) with the following settings: Quant-iT fluorescence protocol, Fluorescence Endpoint read, Excitation 502/10, Emission 523/10, Gain 75, Optics Top, read height 4.5 mm. Remember to substract the blank readings from those well containing DNA (the Quant-iT fluorescence protocol with the Gen5 software provides fluorescence values automatically adjusted to the blanks if the blank wells are indicated).

  e) Using a data graphing software, plot the amount of DNA from the standards (ng) vs the average measured fluorescence (relative fluorescence units; RFUs). Then use a nonlinear (or linear) regression analysis, forcing the Y intercept to 0, to draw a trendline and calculate the slope of the line. Record the slope.

  f) In excel, or equivalent software, calculate the amount of DNA (ng) in your unknown samples by dividing the RFU values by the slope. Then, calculate the concentration of DNA in your samples (ng/µl) by dividing the amount of DNA (ng) by the volume of DNA (µl) added - in this example it would be **5 µl** for the volume as detailed in step (c).
